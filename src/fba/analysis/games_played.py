...
COUNTING_CATEGORIES = [
    ...
    for c in get_counting_configs(DEFAULT_8CAT_CONFIG)
]


def _get_counting_cat_meta(
    category_config: Optional[List[CategoryConfig]] = None,
) -> list[dict]:
    """Return counting category metadata dicts from config."""
    if category_config is not None:
        return [{"key": c.key, "display": c.display, "higher_is_better": c.higher_is_better}
                for c in get_counting_configs(category_config)]
    return COUNTING_CATEGORIES
...
def compute_projected_totals(
    ...
    """
    counting_cats = _get_counting_cat_meta(category_config)
...
def compute_projected_roto_ranks(
    ...
    """Rank teams by projected counting totals, adding percentage base ranks from Yahoo.

    Ranking logic matches normalize.rank_teams_by_category:
      - Best value = highest rank (N = best, 1 = worst)
      - None values get rank 1 (worst)
      - Tie-break by team_name ascending for determinism
      - Respects category directionality

    Percentage category ranks are carried forward from Yahoo's roto_points data
    ...
    counting_cats = get_counting_configs(configs)
    pct_cats = get_percentage_configs(configs)

    n_teams = len(projected_rows)

    # Build Yahoo percentage rank lookup from roto_points
    yahoo_pct_ranks: dict[str, dict[str, Optional[float]]] = {}
    for team in teams:
        name = team.get("team_name", "")
        rp = team.get("roto_points", {})
        yahoo_pct_ranks[name] = {c.key: rp.get(c.key) for c in pct_cats}

    # Rank each counting category
    counting_keys = [f"projected_{c.key}" for c in counting_cats]
    # Build higher_is_better lookup for counting categories
    counting_hib = {f"projected_{c.key}": c.higher_is_better for c in counting_cats}
    ...
    for proj_key in counting_keys:
        ...
        hib = counting_hib.get(proj_key, True)
    ...
    for r in projected_rows:
        ...
        for c in counting_cats:
            proj_key = f"projected_{c.key}"
            rank_val = rank_data.get(proj_key)
            row[f"{c.key}_Rank"] = rank_val
            if rank_val is not None:
                cat_rank_sum.append(rank_val)

        # Percentage ranks from Yahoo base ranks
        for c in pct_cats:
            pct_rank = yr.get(c.key)
            row[f"{c.key}_Rank"] = pct_rank
            if pct_rank is not None:
                cat_rank_sum.append(pct_rank)
